...
export default function WalletPopup({
  ...
  const defaultForm = { title: '', amount: '', type: walletTypes[0]?.id || 'other' };
  const [form, setForm] = useState(defaultForm);
  const firstRef = useRef(null);

  useEffect(() => {
    if (open) {
      setForm(initial ? { title: initial.title, amount: String(initial.amount || ''), type: initial.type } : defaultForm);
    ...
    <div className={styles.overlay} onMouseDown={onClose}>
      <div className={styles.modal} onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <header className={styles.header}>
          <h3 className={styles.h}>{initial ? 'Редагувати гаманець' : 'Додати гаманець'}</h3>
        ...
        <form className={styles.form} onSubmit={submit}>
          <label className={styles.label}>
            Назва
            <input ref={firstRef} name="title" value={form.title} onChange={handleChange} className={styles.input} />
          </label>

          <label className={styles.label}>
            Початкова сума
            <input name="amount" value={form.amount} onChange={handleChange} type="number" className={styles.input} />
          </label>
          ...
          <div className={styles.actions}>
            ...
            <button type="submit" className={styles.btnCreate}>{initial ? 'Зберегти' : 'Створити'}</button>
          </div>
        </form>
      </div>
    </div>,
